...
import DisplayDrawingFunctions.TasksDrawingFunction;
...
public class CreateTasks {
    ...
    CreateTasks(Project workingProject){
        JFrame currentWindow = new JFrame();
        TasksDrawingFunction panel = new TasksDrawingFunction();
        ...
        enterTitle.setBounds(27, 27, 68, 27);
        ...
        //write the title of the forum
        JTextField writeTitle = new JTextField();
        writeTitle.setBounds(95, 27, 460, 27);
        writeTitle.setBackground(BACKGROUND_COLOR);
        writeTitle.setEditable(true);
        panel.add(writeTitle);

        JTextField enterMessage = new JTextField("Task Description:");
        enterMessage.setBounds(27, 78, 110, 25);
        ...
        confirmButton.addActionListener(new ActionListener() {
            ...
            public void actionPerformed(ActionEvent e) {
                if (!writeTitle.getText().equals("") && !writeMessage.getText().equals("")) {
                    workingProject.addTask(writeTitle.getText(), writeMessage.getText());
                    for (int i = 0; i < listTopics.size(); i++) {
                        if (listTopics.get(i).isSelected()) {
                            workingProject.getUncompletedTasks().get(workingProject.getUncompletedTasks().size() - 1).addTopic(workingProject.getTopic(listTopics.get(i).getText()));
                            workingProject.getTopic(listTopics.get(i).getText()).addTask(workingProject.getUncompletedTasks().get(workingProject.getUncompletedTasks().size() - 1));
                        }
                    }
                    Dashboard newDashboard = new Dashboard(workingProject);
                    currentWindow.dispose();
        ...
        currentWindow.setTitle("Create a New Task");
